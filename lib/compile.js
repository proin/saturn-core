...
module.exports = (()=> {
    ...
    lib.node_modules = (src, project_path)=> {
        ...
        for (let i = 0; i < npms.length; i++) {
            ...
            if (fs.existsSync(path.resolve(path.dirname(project_path), npms[i])) === false) {
                result.push(npms[i]);
            }
        }
    ...
    lib.runnable = (project_path)=> {
        ...
        languageModule.r = require('../language-module/r');
        languageModule.torch = require('../language-module/torch');
        languageModule.nodesync = require('../language-module/nodesync');
